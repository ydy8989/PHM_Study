...
df = pd.read_csv("C:/Users/hbee/PycharmProjects/PHM_Study/Auto_Encoder/processminer-rare-event-mts - data.csv")
...
def temporalize(X, y, lookback):
    X = []
    y = []
    for i in range(len(input_X) - lookback - 1):#전체 시퀀스 - lookback - 1만큼의 길이.. 마지막 룩백땜에
        t = []
...
# display(df.iloc[(np.where(np.array(input_y) == 1)[0][0]-5):(np.where(np.array(input_y) == 1)[0][0]+1), ])
lookback = 5  # Equivalent to 10 min of past data. 이게 윈도우 사이즈
